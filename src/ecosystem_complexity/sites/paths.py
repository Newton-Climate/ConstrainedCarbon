"""Repo-root-relative data paths for the site inversion drivers.

The site drivers read inputs that are versioned alongside the code but *not*
packaged with it: per-site config YAMLs under ``configs/multisite/``, the ISRaD
tables and atmospheric ¹⁴C records under ``data/shared/``, and the exported
SoilGrids table under ``notebooks/exports/``. When this code lived in
``notebooks/sites/`` it reached those by walking up from ``__file__``; that
breaks once the same code is imported from an installed package.
