...
from ecosystem_complexity.model.api import ModelOutput, run_model
...
def build_mean_ss_modifier(
    ...
    ft = _f_temp(
        jnp.array(soil_t_np, dtype=jnp.float32), params0.log_Q10[0], T_ref=15.0
    )
    fm = _f_moisture(
    ...
    mean_modifier = (
        mean_modifier if (np.isfinite(mean_modifier) and mean_modifier > 0.05) else 0.05
    )
