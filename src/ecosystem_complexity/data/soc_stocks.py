"""Measured soil-carbon stock constraints from ISRaD and SoilGrids.

These read observational tables and reduce them to a total-column SOC value
with an uncertainty. They are pure data reduction — the only thing they need
from the model is its column depth and pool depth bins, so they take a built
model rather than a site.

The model's *own* steady-state SOC prior is not here: it runs the forward model
and is therefore an inversion concern, in
:mod:`ecosystem_complexity.sites.soc`.
"""
...
from ecosystem_complexity.data.israd_14c import _pool_depth_bins_cm
from ecosystem_complexity.data.israd_observations import add_layer_midpoint
from ecosystem_complexity.data.paths import ISRAD_LAYER, SOILGRIDS_CSV
