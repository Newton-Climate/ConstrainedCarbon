...
def _build_oe_obs_vector(
    ...
    t_pool_idx   : (n_pool_obs,) int32 — time-step indices for pool obs
    pool_col_idx : (n_pool_obs,) int32 — pool-column indices for pool obs
    t_resp_idx   : (n_resp_obs,) int32 — time-step indices for resp obs
    n_pool_obs, n_resp_obs : int
    """
    ...
    if n_resp_obs > 0:
        y       = jnp.concatenate([y_pool, y_resp])
        Se_diag = jnp.concatenate([
            jnp.full(n_pool_obs, sigma_pool ** 2),
            jnp.full(n_resp_obs, sigma_resp ** 2),
        ])
    else:
        y       = y_pool
        Se_diag = jnp.full(n_pool_obs, sigma_pool ** 2)

    return y, Se_diag, t_pool_idx, pool_col_idx, t_resp_idx, n_pool_obs, n_resp_obs
...
def optimize_oe(
    ...
    (y, Se_diag,
     ...
     n_pool_obs, n_resp_obs) = _build_oe_obs_vector(
        observations, model, sigma_pool, sigma_resp)
    ...
    def _forward(x_vec):
        ...
        y_pred_pool = out.delta14C[t_pool_idx, pool_col_idx]

        if n_resp_obs > 0:
            tau_v     = jnp.exp(p.log_tau)
            w         = out.C12 / (tau_v[None, :] + 1e-30)
            d14c_resp = (out.delta14C * w).sum(-1) / (w.sum(-1) + 1e-30)
            return jnp.concatenate([y_pred_pool, d14c_resp[t_resp_idx]])
        return y_pred_pool
