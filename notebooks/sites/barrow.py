...
Contains all Barrow-specific data helpers, the full OE5 inversion workflow,
and the site figure.  Imported by ``notebooks/barrow_model.py``.
...
# Depth-to-pool mapping for Vaughn pore-gas Δ¹⁴C
...
def build_pool_14C_obs(rc_path: str, forcing_time, pool_names: list) -> dict:
    """
...
def run_optimal_inversion() -> dict:
    """
    Run the Barrow OE1–OE5 inversion sequence.
    ...
    delta14C_obs  = build_pool_14C_obs(BARROW_RC_PATH, forcing.time, idx.pool_names)
    ...
    extra_blocks: list = carbon_sum_blocks   # no ISRaD for Barrow
...
def make_figure(r: dict, out_path: str | None = None):
    """Save the 8-panel OE comparison figure for Barrow."""
    if out_path is None:
        out_path = _wt("notebooks/barrow_model.png")
    make_oe5_figure(
        r=r,
        site_title="Barrow, Alaska — 3-Pool Optimal Model  (active + slow + passive)",
        site_subtitle="OE5 adds FluxNet ER → annual Rh constraint (f_hetero≈0.80, σ=20%)",
