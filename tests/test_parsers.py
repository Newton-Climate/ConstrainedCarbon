...
def _make_hua_csv() -> str:
    ...
    for y in years:
        if 1955 <= y <= 1970:
            ...
            vals.append(0.0)
    df = pd.DataFrame({"Year.AD": years, "NH14C": vals, "SH14C": [v * 0.9 for v in vals]})
    return df.to_csv(index=False)
...
def test_intcal20_parsing():
    ...
    try:
        years, d14c = load_full_14C_record(
            hua_path, graven_path, intcal_path,
            start_year=1500.0, end_year=2019.0,
        )
        idx_1950 = np.argmin(np.abs(years - 1950.0))
        assert -50 < d14c[idx_1950] < 50, (
...
def test_align_to_layers_depth_weighted(hf_config):
    ...
    assert abs(float(vals[0]) - 10.0) < 0.01, (
...
def test_israd_depth_convention(hf_config):
    ...
    vals, _ = align_to_layers(
        tops_m, bots_m, [50.0, 30.0], [5.0, 5.0], hf_config
    )
...
def test_attach_atm14C(hf_config):
    ...
    try:
        ...
        assert jnp.all(jnp.isnan(forcing.delta14C_atm)), \
        ...
        assert not jnp.any(jnp.isnan(forcing2.delta14C_atm)), \
...
def test_validate_forcing_warns_nan(hf_config):
    ...
    try:
        ...
        assert any("air_temp" in w for w in warnings), \
